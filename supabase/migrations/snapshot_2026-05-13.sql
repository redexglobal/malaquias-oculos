...
USING (public._get_papel(auth.uid()) = 'admin' OR id = auth.uid());
...
FOR SELECT TO authenticated
USING (public._get_papel(auth.uid()) = 'admin' OR cadastrado_por = auth.uid());
...
ORDER BY CASE p.tipo_ponto WHEN 'fixo' THEN 1 WHEN 'móvel' THEN 2 WHEN 'movel' THEN 2 ELSE 3 END, p.numero_ponto;

-- 8) View v_pontos_oculos
...
RETURNS TABLE(
  ...
  online_at timestamptz, online boolean, cadastros_no_dia bigint, media_dor numeric, cadastros_dor_alta bigint
)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT p.id, p.nome, p.nome_ponto, p.numero_ponto, p.tipo_ponto, p.status, p.online_at,
    (p.online_at IS NOT NULL AND p.online_at > NOW() - INTERVAL '5 minutes'),
    COALESCE(COUNT(pf.id), 0),
    ROUND(AVG(pf.nivel_dor)::numeric, 1),
    COALESCE(COUNT(CASE WHEN pf.nivel_dor >= 7 THEN 1 END), 0)
  FROM public.profiles p
  LEFT JOIN public.pacientes_fibro pf ON pf.cadastrado_por = p.id AND pf.criado_em::date = p_data
  WHERE p.papel = 'colaborador' AND p.projeto = 'fibromialgia' AND p.ativo = true
  GROUP BY p.id, p.nome, p.nome_ponto, p.numero_ponto, p.tipo_ponto, p.status, p.online_at
  ORDER BY CASE p.tipo_ponto WHEN 'fixo' THEN 1 WHEN 'móvel' THEN 2 WHEN 'movel' THEN 2 ELSE 3 END, p.numero_ponto;
$$;
...
RETURNS TABLE(
  id uuid, nome text, nome_ponto text, numero_ponto int, tipo_ponto text, status text,
  online_at timestamptz, online boolean, cadastros_no_dia bigint, media_avaliacao numeric, valor_total numeric
)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT p.id, p.nome, p.nome_ponto, p.numero_ponto, p.tipo_ponto, p.status, p.online_at,
    (p.online_at IS NOT NULL AND p.online_at > NOW() - INTERVAL '5 minutes'),
    COALESCE(COUNT(po.id), 0),
    ROUND(AVG(po.avaliacao)::numeric, 1),
    COALESCE(SUM(po.valor_pedido), 0)
  FROM public.profiles p
  LEFT JOIN public.pacientes_oculos po ON po.cadastrado_por = p.id AND po.criado_em::date = p_data
  WHERE p.papel = 'colaborador' AND p.projeto = 'oculos' AND p.ativo = true
  GROUP BY p.id, p.nome, p.nome_ponto, p.numero_ponto, p.tipo_ponto, p.status, p.online_at
  ORDER BY CASE p.tipo_ponto WHEN 'fixo' THEN 1 WHEN 'móvel' THEN 2 WHEN 'movel' THEN 2 ELSE 3 END, p.numero_ponto;
$$;
